...
interface QuizHistoryItem {
  ...
  quizId: {
    ...
    module: string;
    chapter: string;
    questions: Question[]; // only the questions this student actually answered
  };
  score: number;
...
const UserDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        const res = await axios.get(`${API}/api/quizresult/stats/${user._id}`, { headers });
    ...
    const fetchTopicStats = async () => {
      try {
        const res = await axios.get(`${API}/api/quizresult/stats-by-topic/${user._id}`, { headers });
    ...
    const fetchHistory = async () => {
      try {
        const res = await axios.get(`${API}/api/quizresult/user/${user._id}`, { headers });
  ...
  useEffect(() => {
    ...
    if (historyFilter.searchTerm) {
      const s = historyFilter.searchTerm.toLowerCase();
      filtered = filtered.filter(
        (item) =>
          item.quizId?.module?.toLowerCase().includes(s) ||
          item.quizId?.chapter?.toLowerCase().includes(s)
      );
    }
  ...
  const renderHistoryFilters = () => (
    <>
      ...
      <div className={`mb-6 p-4 lg:p-6 bg-white rounded-lg border border-gray-200 transition-all ${showFilters || isDesktop ? "block" : "hidden"}`}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          <div>
            <label className="block text-sm font-semibold mb-2 lg:mb-3 text-gray-700">Filter by Topic</label>
            <select
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 lg:mb-3 text-gray-700">Sort by</label>
            <select
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 lg:mb-3 text-gray-700">Search</label>
            <input
              type="text"
          ...
          <div className="flex items-end">
            <button
              onClick={() => { setHistoryFilter({ topic: "all", sortBy: "date", searchTerm: "" }); setShowFilters(false); }}
            ...
            {filteredHistory.length > 0 ? (
              filteredHistory.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">{item.correctAnswers}/{item.totalQuestions}</td>
          ...
          filteredHistory.map((item, index) => (
            <div key={index} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-gray-600">Score: </span><span className="font-medium">{item.score} pts</span></div>
                <div><span className="text-gray-600">Accuracy: </span><span className="font-medium">{Math.round((item.correctAnswers / item.totalQuestions) * 100)}%</span></div>
                <div><span className="text-gray-600">Date: </span><span className="font-medium">{new Date(item.createdAt).toLocaleDateString()}</span></div>
                <div><span className="text-gray-600">Correct: </span><span className="font-medium">{item.correctAnswers}/{item.totalQuestions}</span></div>
              </div>
            </div>
          ))
        ) : (
  ...
  // ── Review Modal ───────────────────────────────────────
  // Fixed: matches userAnswers to questions using .toString() on both sides
  // Fixed: only shows questions the student actually answered (filtered on backend)
  const renderReviewModal = () => {
    if (!selectedQuiz || !showReview) return null;

    const questions = selectedQuiz.quizId?.questions || [];
    const userAnswers = selectedQuiz.userAnswers || [];

    // Build a fast lookup map: questionId string → UserAnswer
    const answerMap = new Map<string, UserAnswer>();
    userAnswers.forEach((ua) => {
      answerMap.set(ua.questionId.toString(), ua);
    });
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold text-gray-800 capitalize">
                    {topicDisplayNames[selectedQuiz.quizId?.module] ||
                      selectedQuiz.quizId?.module?.replace(/-/g, " ")}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">Chapter</p>
                  <p className="font-semibold text-gray-800 capitalize">
                    {selectedQuiz.quizId?.chapter?.replace(/-/g, " ") || "Module Quiz"}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-800">
                    {Math.round((selectedQuiz.correctAnswers / selectedQuiz.totalQuestions) * 100)}%
                  </p>
                </div>
                <div>
            ...
            {questions.length === 0 ? (
              ...
              <div className="space-y-6 lg:space-y-8">
                {questions.map((question, index) => {
                  // ── Fixed lookup: compare as strings ──
                    ...
                    >
                      ...
                      {question.imageUrl && (
                        <div className="mb-4 lg:mb-6">
                          <img src={question.imageUrl} alt="Question" className="max-w-full h-auto rounded-lg shadow-md" />
                        </div>
                      )}
                      ...
                      {question.type === "MCQ" && question.options && (
                        <div className="space-y-2 lg:space-y-3 mb-4 lg:mb-6">
                          {question.options.map((option, optIndex) => {
                            ...
                            let optionClass = "p-3 lg:p-4 rounded-lg border-2 transition-all ";
                            if (isCorrectOption) optionClass += "bg-green-50 border-green-300";
                            else if (isUserAnswer && !isCorrectOption) optionClass += "bg-red-50 border-red-300";
                              ...
                              <div key={optIndex} className={optionClass}>
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                  ...
                                  <div className="flex items-center space-x-2 ml-6 sm:ml-0">
                                    {isUserAnswer && (
                                      <span className="text-blue-600 font-medium text-sm">(Your choice)</span>
                                    )}
                                    {isCorrectOption && (
                      ...
                      {/* No answer recorded fallback */}
                      {!userAnswer && (
                        <div className="mb-4 p-4 bg-gray-50 border-l-4 border-gray-300 rounded-r-lg">
                          <p className="text-gray-600 text-sm">No answer recorded for this question.</p>
                        </div>
                      )}
                      ...
                      {question.solutionImageUrl && (
                        <div className="mb-4 lg:mb-6">
                          ...
                          <img src={question.solutionImageUrl} alt="Solution" className="max-w-full h-auto rounded-lg shadow-md" />
                        </div>
                      )}
                    </div>
                  );
