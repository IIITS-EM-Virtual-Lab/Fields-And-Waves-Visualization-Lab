...
function useFresnel(
  thetaI_deg: number,
  er1: number, er2: number,
  mr1: number, mr2: number,
  pol: Pol, pec: boolean
) {
  return useMemo(() => {
...
function endPointFromNormal(origin: Pt, angleFromNormalRad: number, len: number, horizontalSign: 1 | -1): Pt {
  // Angle measured from the upward normal. SVG y increases downward.
...
/* ================= WaveReflection2D (function declaration) ===== */
interface WaveReflection2DProps {
  thetaI?: number; er1?: number; er2?: number; mr1?: number; mr2?: number;
  pol?: Pol; pec?: boolean; showWavefronts?: boolean;
  width?: number; height?: number;
}

function WaveReflection2D({
  thetaI = 30, er1 = 1, er2 = 2.25, mr1 = 1, mr2 = 1,
  pol = "p", pec = false, showWavefronts = true,
  width = 980, height = 480,
}: WaveReflection2DProps) {
  const midX = width / 2;
  const midY = height / 2;
  ...
  return (
    <svg className="wave2d-svg" width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
      ...
      {/* Rays */}
      <line x1={incStart.x} y1={incStart.y} x2={hit.x} y2={hit.y} stroke="#2563eb" strokeWidth="3" markerEnd="url(#ah-blue)" />
      <text x={incStart.x + 10} y={incStart.y + 18} className="tag" fill="#2563eb">Incident</text>

      <line x1={hit.x} y1={hit.y} x2={refEnd.x} y2={refEnd.y} stroke="#10b981" strokeWidth="3" markerEnd="url(#ah-green)" />
      <text x={hit.x + 14} y={hit.y - 10} className="tag" fill="#10b981">Reflected</text>

      {!f.tir && !pec && (
        <>
          <line x1={hit.x} y1={hit.y} x2={trnEnd.x} y2={trnEnd.y} stroke="#f59e0b" strokeWidth="3" markerEnd="url(#ah-orange)" />
          <text x={hit.x + 14} y={hit.y + 18} className="tag" fill="#f59e0b">Transmitted</text>
        </>
      )}

      {/* Angle arcs */}
      <path d={`M ${midX} ${midY} A 60 60 0 0 1 ${midX - Math.sin(thI) * 60} ${midY - Math.cos(thI) * 60}`}
            fill="none" stroke="#111" strokeWidth="2" />
      <text x={midX - Math.sin(thI) * 72} y={midY - Math.cos(thI) * 72} className="tag">θᵢ</text>

      {!f.tir && !pec && (
        <>
          <path d={`M ${midX} ${midY} A 60 60 0 0 0 ${midX - Math.sin(thT) * 60} ${midY + Math.cos(thT) * 60}`}
                fill="none" stroke="#111" strokeWidth="2" />
          <text x={midX - Math.sin(thT) * 72} y={midY + Math.cos(thT) * 72} className="tag">θₜ</text>
        </>
      )}

      {/* Energy arrows (length ~ √power) */}
      ...
      {showWavefronts && (
        <>
          {Array.from({ length: 9 }).map((_, i) => {
            ...
            return (
              <line key={`wf1-${i}`} x1={midX - 320 - dx} y1={y} x2={midX + 320 - dx} y2={y}
          ...
          {!f.tir && !pec && Array.from({ length: 9 }).map((_, i) => {
            ...
            return (
              <line key={`wf2-${i}`} x1={midX - 320 - dx} y1={y} x2={midX + 320 - dx} y2={y}
...
const WaveReflection: React.FC = () => {
  ...
  return (
    <div className="wave-container">
      ...
      <div className="visualizer-container">
        <div className="controls">
          ...
          <label className="block text-sm mb-1">
            Angle θᵢ (deg):
            <input type="range" min={0} max={89} value={thetaI}
      ...
      {/* ---------- Theory (KaTeX rendered correctly) ---------- */}
      <div className="topic">
