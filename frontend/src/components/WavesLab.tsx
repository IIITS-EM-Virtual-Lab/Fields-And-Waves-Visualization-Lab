...
function EMPlaneWave({
  ...
  return (
    <group>
      ...
      <Axes size={3.2} />
      <Arrow
        from={new THREE.Vector3(0, 0, -6.5)}
...
function EMTriadAtZ({
  ...
  useFrame(() => {
    ...
    eAH.setLength(
      Emag,
      Math.max(0.06, Emag * 0.25),
      Math.max(0.06, Emag * 0.15),
    );

    // H = (1/η) k̂ × E ; k̂=+z → (-Ey, Ex, 0)/η
    const Hvec = new THREE.Vector3(
      -Ey / Math.max(1e-9, eta),
      Ex / Math.max(1e-9, eta),
      0,
    );
    ...
    hAH.setDirection(
      Hvec.length() === 0
        ? new THREE.Vector3(0, 1, 0)
        : Hvec.clone().normalize(),
    );
    hAH.setLength(
      Hmag,
      Math.max(0.06, Hmag * 0.25),
      Math.max(0.06, Hmag * 0.15),
    );
    ...
    sAH.setLength(
      Smag * 0.6,
      Math.max(0.06, Smag * 0.15),
      Math.max(0.06, Smag * 0.1),
    );
  });
  ...
  return (
    <group>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.001, 0]}>
        ...
        <meshBasicMaterial color="#e5e7eb" opacity={0.15} transparent />
      </mesh>
      {trail.current.length > 1 && (
        <DreiLine points={trail.current} color="#92400e" lineWidth={2.4} />
      )}
  ...
  return (
    <group>
      ...
      {points.current.length > 1 && (
        <DreiLine points={points.current} color="#0f766e" lineWidth={2.6} />
      )}
      {nodes.map((z) => (
...
function AttenuatedWave({
  ...
  const envNeg: THREE.Vector3[] = useMemo(
    () => envPos.map((p) => p.clone().multiplyScalar(-1)),
    [envPos],
  );
  ...
  return (
    <group>
      ...
      {pts.current.length > 1 && (
        <DreiLine points={pts.current} color="#9d174d" lineWidth={2.6} />
      )}
      <DreiLine points={envPos} color="#92400e" lineWidth={1.4} />
      <DreiLine points={envNeg} color="#92400e" lineWidth={1.4} />
      ...
      >
        <div style={legendBox}>
          ...
          <div>
            E(z,t) = A e<sup>-αz</sup> sin(ωt − kz)
          </div>
...
export default function WavesLab(): JSX.Element {
  ...
  const [pol, setPol] = useState<"linear" | "circular" | "elliptical">(
    "linear",
  );
    ...
    >
      ...
      <ControlsBar>
        ...
        >
          <label style={label}>
            ...
            <select
              value={moduleKey}
              onChange={(e) => setModuleKey(e.target.value as any)}
              style={select}
            >
            ...
            <>
              ...
              <label style={label}>
                ...
                <input
                  type="number"
                  value={eta}
                  onChange={(e) => setEta(+e.target.value || 377)}
          ...
          {moduleKey === "standing" && (
            <label style={label}>
              ...
              <input
                ...
                step={0.05}
                value={Astand}
                onChange={(e) => setAstand(+e.target.value)}
              />
            ...
            <>
              <label style={label}>
                ...
                <input
                  ...
                  step={0.05}
                  value={Aatt}
                  onChange={(e) => setAatt(+e.target.value)}
                />
      ...
      <div className="wave-visual">
        ...
        >
          ...
          {moduleKey === "standing" && (
            <StandingWave A={Astand} k={k} freq={freq} paused={paused} />
          )}          
          {moduleKey === "attenuated" && (
            <AttenuatedWave
              A={Aatt}
...
function Header() {
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <div style={{ color: "#1f2937", fontSize: 15 }}>
        Explore transverse EM waves (E–H–k), polarization, standing waves,
        longitudinal waves, and attenuation.
      </div>
    </div>
  );
}
  ...
  return (
    <div
      style={{
        ...
        background: "rgba(255, 255, 255, 0.88)", // white w/ transparency
        color: "#0b1220",
        borderRadius: 14,
        padding: 12,
        display: "flex",
        ...
        gap: 12,
        backdropFilter: "blur(6px)", // subtle frosted-glass feel
      }}
    >
      {children}
    </div>
...
const legendTitleDark: React.CSSProperties = {
  marginBottom: 6,
  fontWeight: 900,
  color: "#000",
};
