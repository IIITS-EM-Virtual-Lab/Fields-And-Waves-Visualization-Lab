  ...
  return (
    <div className="flex flex-col items-center gap-4 p-2 sm:p-4 w-full">
      <div
        ...
        style={{ zIndex: 0 }}
      >
        <Canvas style={{ height: '100%', width: '100%' }} camera={{position: [1, 3, 7]}}>
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 text-sm mt-4 w-full max-w-md">
        <h1 className="w-full sm:w-auto font-semibold mb-2 sm:mb-0">Input Coordinates for Vector-C:</h1>
        <label className="flex-1 flex items-center">
          <span className="mr-1">X:</span>
          <input type="number" value={x3} onChange={(e) => setX3(Number(e.target.value))} className="border p-1 w-full min-w-0" />
        </label>
        <label className="flex-1 flex items-center">
          <span className="mr-1">Y:</span>
          <input type="number" value={y3} onChange={(e) => setY3(Number(e.target.value))} className="border p-1 w-full min-w-0" />
        </label>
        <label className="flex-1 flex items-center">
          <span className="mr-1">Z:</span>
          <input type="number" value={z3} onChange={(e) => setZ3(Number(e.target.value))} className="border p-1 w-full min-w-0" />
        </label>
      </div>
