...
const TransformerEMFVisualizer: React.FC = () => {
  ...
  const phiInstant =
    (v1Peak / (n1 * omega)) * Math.sin(omega * time);
  const v2Instant =
    -n2 * (v1Peak / n1) * Math.cos(omega * time);
  ...
  // Main Transformer Canvas Drawing
  useEffect(() => {
    ...
    // Magnetic Flux Circulation Lines
    const fluxFactor = Math.sin(omega * time);
    const fluxAlpha = Math.min(
      0.95,
      Math.abs(fluxFactor) * 0.8 + 0.2
    );

    const fluxColor =
      fluxFactor >= 0
        ? `rgba(2, 132, 199, ${fluxAlpha})`
        : `rgba(225, 29, 72, ${fluxAlpha})`;

    const fluxMidX1 = coreX + coreThick / 2;
    const fluxMidX2 =
      coreX + coreOuterW - coreThick / 2;
    const fluxMidY1 = coreY + coreThick / 2;
    const fluxMidY2 =
      coreY + coreOuterH - coreThick / 2;

    ctx.strokeStyle = fluxColor;
    ctx.lineWidth = 3.5;
    ctx.setLineDash([8, 6]);
    ctx.lineDashOffset =
      -time * 50 * (fluxFactor >= 0 ? 1 : -1);
    ...
    const primaryTurnCount = Math.min(
      14,
      Math.max(4, Math.floor(n1 / 4))
    );

    const primaryCoilY1 =
      coreY + coreThick + 10;
    const primaryCoilY2 =
      coreY + coreOuterH - coreThick - 10;

    const primarySpacing =
      (primaryCoilY2 - primaryCoilY1) /
      primaryTurnCount;

    for (let i = 0; i < primaryTurnCount; i++) {
      const ty =
        primaryCoilY1 + i * primarySpacing;
    ...
    const secondaryTurnCount = Math.min(
      18,
      Math.max(4, Math.floor(n2 / 4))
    );

    const secondaryCoilY1 =
      coreY + coreThick + 8;
    const secondaryCoilY2 =
      coreY + coreOuterH - coreThick - 8;

    const secondarySpacing =
      (secondaryCoilY2 - secondaryCoilY1) /
      secondaryTurnCount;

    for (let i = 0; i < secondaryTurnCount; i++) {
      const ty =
        secondaryCoilY1 + i * secondarySpacing;
    ...
    const loadCenterY =
      (secondaryCoilY1 + secondaryCoilY2) / 2;
    ...
    const bulbGlow = Math.min(
      1.0,
      (Math.abs(v2Instant) / 200) * 0.9 + 0.1
    );
    ...
    ctx.fillText(
      `V₂ = ${Math.abs(v2Peak).toFixed(0)}V AC`,
      loadCenterX,
      loadCenterY + 38
    );
    ...
    ctx.fillText(
      'Iron Core (μ_r ≫ 1)',
      (coreX + innerX + coreInnerW) / 2,
      innerY + coreInnerH / 2 - 8
    );

    ctx.fillStyle =
      fluxFactor >= 0 ? '#0284c7' : '#e11d48';

    ctx.fillText(
      `Flux Φ(t) = ${(phiInstant * 1000).toFixed(1)} mWb`,
      (coreX + innerX + coreInnerW) / 2,
      innerY + coreInnerH / 2 + 10
    );
  }, [
    time,
    v1Peak,
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col gap-3 text-xs">
            ...
            <div>
              ...
              <input
                ...
                value={v1Peak}
                onChange={(e) =>
                  setV1Peak(parseInt(e.target.value))
                }
